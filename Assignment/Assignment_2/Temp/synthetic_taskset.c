...
//				cout<<"   "<<U <<" task number : "<<i<<" "<<taskset[i].wcet<<" "<<taskset[i].deadline<<" "<<taskset[i].period<<" "<<endl;
			}
...
/*			for(int i=0;i<n;i++){
				ret_array[i]=(random[i]*U)/sum;		//calculates utilization
//				cout<<ret_array[i]<<endl;
			}*/
			for(int i=0;i<n;i++){
				taskset[i].period=(rand()%numberoftasks) + 1;	//set random period
...
//				cout<<"   "<<U <<" task number : "<<i<<" "<<taskset[i].wcet<<" "<<taskset[i].deadline<<" "<<taskset[i].period<<" "<<endl;
		...
		float* Uunifast(int n,float U,int M,int type){
			...
			for(int i=0;i<n;i++){
				...
				taskset[i].wcet=(ret_array[i])*taskset[i].period; 		// set wcet with respect to its utilization and random period. 
			//	taskset[i].deadline=((rand()%(int)taskset[i].period)+taskset[i].wcet)%(int)taskset[i].period;	// set random deadline
				if(type==0){
...
//				cout<<" dont know if task number : "<< task_no << " is schedulable" <<endl;
			...
			/*if(util_by_tasks>=1){		// if density is more than one it is supposed to do time response analysis
//				cout<<"LST -- utilization based analysis inconclusive, hence response based analysis : " <<tasks[task_no].slack_time<<endl;
				int resp_tim=resp_tim_for_task(task_no);		// get resp of task_no in presence of higher priority tasks
				if(tasks[task_no].is_sched=resp_tim<=tasks[task_no].deadline){	
				no_task_schedulable++;
				}else {		
...
//	rmfile<<"pic hwaade pe kutta kata"<<endl;
try{	//if the file is not present
	...
	//		fp.ignore(256,'\n');
		...
		*/	synthetic_taskset set=synthetic_taskset(jobs);	// generates jobs number of synthetic task.
			...
			dm dm_analysis=dm(jobs);	
			LST lst_analysis=LST(jobs);	
		//	set.Uscaling(12,8.0*(pow(2,1.0/8)-1.0));		// for 12 tasks and total utilization 1
	//		set.Ufitting(12,8.0*(pow(2,1.0/8)-1.0));		// for 12 tasks and total utilization 1
	//		set.Ufitting(12,0.9);		// for 12 tasks and total utilization 1
			for(int j=0;j<10;j++){		//for different set of 
				...
				//	cout<<"rm -- schedulability res for taskset-"<<i<<" :  "<<" number of tasks schedulable are: "<<rm_analysis.get_sched_tasks()<<endl;
						rmcount++;
					}
				//	cout<<"rm -- schedulability res for taskset-"<<i<<" :  "<<" number of tasks schedulable are: "<<rm_analysis.get_sched_tasks()<<endl;
				...
				//	cout<<"dm -- schedulability res for taskset-"<<i<<" :  "<<" number of tasks schedulable are: "<<dm_analysis.get_sched_tasks()<<endl;
					if(lst_analysis.taskset_schedulable()){
						lstcount++;
					}
				//	cout<<"LST -- schedulability res for taskset-"<<i<<" : "<<" number of tasks schedulable are: "<<lst_analysis.get_sched_tasks()<<endl;
				}
				
	//				cout<<"rm -- schedulability res for taskset-"<<i<<" :  "<<" number of taskset schedulable are: "<<rmcount<<endl;
					rmfile[file_no]<<util<<" : "<<rmcount<<endl;
					dmfile[file_no]<<util<<" : "<<dmcount<<endl;
					lstfile[file_no]<<util<<" : "<<lstcount<<endl;
	//				file_no++;
	//				cout<<"dm -- schedulability res for taskset-"<<i<<" :  "<<" number of taskset schedulable are: "<<dmcount<<endl;
	//				cout<<"lst -- schedulability res for taskset-"<<i<<" :  "<<" number of taskset schedulable are: "<<lstcount<<endl;
