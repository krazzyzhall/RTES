...
cout << "format : a.out <name of parameter file>" << endl; //if improper number of parameter
}}
else if(argc==1){
		...
		for(int i=1;(i<=4);i*=2){
			...
			for(int j=0;j<10;j++){		//for different set of 
				...
				for(int z=0;z<10000;z++){	//for generating 10000 times
					...
					for(int t=0;(t<rm_analysis.get_no_tasks());t++){
					/*	cout<<"rm -- taskset : "<<i<<" task number : "<<t+1<<" schedulability res : " <<
					*/	rm_analysis.is_bounded(t);
					/*	<<endl;
						cout<<"dm -- taskset : "<<i<<" task number : "<<t+1<<" schedulability res : "<< 
					*/	dm_analysis.is_bounded(t);
					/*	<<endl;
						cout<<"LST-- taskset : "<<i<<" task number : "<<t+1<<" schedulability res : "<<
					*/	lst_analysis.is_bounded(t);
					//	<<endl;
					}
					...
					if(dm_analysis.taskset_schedulable()){
						...
						printf("dm:is not schedulable\n");
					}
					if(lst_analysis.taskset_schedulable()){
						printf("lst:is schedulable\n");
						lstcount++;
					}
					else{
						printf("lst:is not schedulable\n");
					}
				}
